"use strict";
class CalculadoraRPN {
    constructor() {
        this.pila = new Array();
    }
    pantalla(digito) {
        let pantalla = document.getElementById("pantalla");
        
        if (pantalla.value === '0') pantalla.value = digito; 
        else pantalla.value += digito;
    }

    anadirOperando(operando) {
        if (this.pila.length >= 2) {
            const primerValor = this.pila.pop();
            const segundoValor = this.pila.pop();
            const resultado = eval(primerValor + operando + segundoValor);
            this.pila.push(resultado);
            document.getElementById("resultado").value = resultado;
        } else {
            alert('Primero añade los numeros y luego el operando');
        }
    }

    anadirNumero(numero) {
        if (!Number.isNaN(numero)) this.pila.push(numero);
        else alert('El caracter que estas intentando añadir no se corresponde a un número');
    }

    enter() {
        this.numero = document.getElementById("pantalla").value;
        this.anadirNumero(this.numero);
        document.getElementById("pantalla").value = 0;
    }

    seno(){
        if(this.pila.length >= 1) this.calcularSeno(this.pila.pop());
        else this.error();
    }

    coseno(){
        if(this.pila.length >= 1) this.introducir(Math.cos(this.pila.pop()));
        else this.error();
    }

    tangente(){
        if(this.pila.length >= 1) this.introducir(Math.tan(this.pila.pop()));
        else this.error();
    }

    calcularFactorial(){
        if(this.pila.length >= 1) this.introducir(this.factorial(this.pila.pop()));
        else this.error();
    }

    factorial(n){
        if(n < 0) return -1;
        if(n == 0) return 1;
        return n * this.factorial(n-1);
    }

    exponencial(){
        if(this.pila.length >= 1) this.introducir(Math.exp(this.pila.pop()));
        else this.error();  
    }

    introducir(res){
        document.getElementById("resultado").value = res;
        document.getElementById("pantalla").value = 0;
        this.pila.push(this.res);
    }

    error(){
        alert('No hay elementos en la pila para realizar la operación.');
    }

    limpiar() {
        this.pila = [];
        document.getElementById("pantalla").value = 0;
        document.getElementById("resultado").value = 0;
    }
}
class CalculadoraEspecializada extends CalculadoraRPN{
    gramos = 0;
    min = 0;
    minFlag = false;
    gFlag = false;

    actividades = new Map([
        ["correr",9.2],
        ["bici", 6.4],
        ["caminar",4],
        ["patinar",10],
        ["nadar",12.4],
        ["baile",13.3],
        ["escalada",17.5],
        ["futbol",7],
        ["tenis", 9],
        ["gimnasio",7.4]
    ]);
    comidas = new Map([
        ["manzana",0.55],
        ["arroz", 1.29],
        ["pasta",3.47],
        ["pollo",1.61],
        ["jamon",1.36],
        ["lechuga",0.125],
        ["ternera",2.29],
        ["merluza",0.72],
        ["salmon",1.82],
        ["boletus",0.26]
    ]);
    setGramos(gramos){
        this.gramos += gramos;
        this.gFlag = true;
        document.getElementById("pantalla").value = gramos + " g";
    }
    setMinutos(min){
        this.min += min;
        this.minFlag = true;
        document.getElementById("pantalla").value = min + " min";
    }
    addComida(comida){
        if(this.gFlag){
            let kcal = this.comidas.get(comida)*this.gramos
            document.getElementById("pantalla").value = kcal + " kcal";
            this.gFlag = false;
            this.gramos = 0;
            this.anadirNumero(kcal);
        }
        else if(this.minFlag) alert('Debe seleccionar una actividad.');
        else alert('Debe seleccionar primero la cantidad de comida');
    }
    addActividad(actividad){
        if(this.minFlag){
            let kcal = (-1)*this.actividades.get(actividad)*this.min
            document.getElementById("pantalla").value = kcal + " kcal";
            this.minFlag = false;
            this.min = 0;
            this.anadirNumero((-1)*this.actividades.get(actividad)*this.min);
        } 
        else if(this.gFlag)  alert('Debe seleccionar una comida.');
        else alert('Debe seleccionar primero la duracion de la actividad');
    }
    calcularBalance(){
        this.anadirOperando('+');
    }
    calcularIMC(){
        let peso = document.getElementById("peso").value;
        let altura = document.getElementById("altura").value;
        
        if(peso === 0 || peso === undefined || peso === null || peso < 0) alert('Introduzca su peso');
        else if(altura === 0 || altura === undefined || altura === null || altura < 0) alert('Introduzca su altura');
        else document.getElementById("imc").value = peso / (altura*altura);
    }
}
document.addEventListener('keydown', (event) => {
    const keyName = event.key;
    switch(keyName.toLowerCase()){
        case '1':
            calculadora.setGramos(25);
            break;
        case '2':
            calculadora.setGramos(50);
            break;
        case '3':
            calculadora.setGramos(75);
            break;
        case '4':
            calculadora.setGramos(100);
            break;
        case '5':
            calculadora.setGramos(150);
            break;
        case '6':
            calculadora.setMinutos(5);
            break;
        case '7':
            calculadora.setMinutos(15);
            break;
        case '8':
            calculadora.setMinutos(30);
            break;
        case '9':
            calculadora.setMinutos(45);
            break;
        case '0':
            calculadora.setMinutos(60);
            break;
        case 'm':
            calculadora.addComida('manzana');
            break;
        case 'a':
            calculadora.addComida('arroz');
            break;
        case 'p':
            calculadora.addComida('pasta');
            break;
        case 'l':
            calculadora.addComida('pollo');
            break;
        case 'j':
            calculator.anadirOperando('jamon');
            break;
        case 'h':
            calculadora.addComida('lechuga');
            break;
        case 't':
            calculadora.addComida('ternera');
            break;
        case 'z':
            calculadora.addComida('merluza');
            break;
        case 's':
            calculadora.addComida('salmon');
            break;
        case 'o':
            calculadora.addComida('boletus');
            break;
        case 'c':
            calculadora.addActividad('correr');
            break;
        case 'b':
            calculadora.addActividad('bici');
            break;
        case 'i':
            calculadora.addActividad('caminar');
            break;
        case 'r':
            calculadora.addActividad('patinar')
            break;
        case 'n':
            calculadora.addActividad('nadar');
            break;
        case 'e':
            calculadora.addActividad('escalada');
            break;
        case 'k':
            calculadora.addActividad('baile');
            break;
        case 'f':
            calculadora.addActividad('futbol');
            break;
        case 't':
            calculadora.addActividad('tenis');
            break;
        case 'g':
            calculadora.addActividad('gimnasio');
            break;
        default:
            break;
    }
});
let calculadora = new CalculadoraEspecializada();